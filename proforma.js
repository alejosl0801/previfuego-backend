...
// ── CATÁLOGO: PRODUCTOS AÑADIDOS POR EL ADMIN (#29) ──────────
// Persistencia espejo de ACCESORIOS: los productos creados desde Config se
// guardan en localStorage y se fusionan en PF_PRODUCTOS_DIST al cargar.
// Los productos hardcodeados de arriba NO se tocan ni se pueden borrar.
var PF_PRODUCTOS_EXTRA_KEY = "pf_productos_extra";

function pfCargarProductosExtra() {
  var extra = [];
  try { extra = JSON.parse(localStorage.getItem(PF_PRODUCTOS_EXTRA_KEY) || "[]"); } catch(e) { extra = []; }
  return Array.isArray(extra) ? extra : [];
}

function pfIdProductoExiste(id) {
  return PF_PRODUCTOS_DIST.some(function(p){ return p.id === id; });
}

// Devuelve {ok:true, producto} o {ok:false, error:"..."}
function pfGuardarProductoCatalogo(obj) {
  var id = (obj.id || "").toString().trim().toUpperCase().replace(/\s+/g, "");
  if (!id) return { ok:false, error:"El ID es obligatorio." };
  if (pfIdProductoExiste(id)) return { ok:false, error:"Ya existe un producto con el ID " + id + "." };
  var nombre = (obj.nombre || "").toString().trim();
  if (!nombre) return { ok:false, error:"El nombre es obligatorio." };
  var pbase = parseFloat(obj.pbase);
  var punit = parseFloat(obj.punit);
  if (isNaN(pbase) || pbase < 0) return { ok:false, error:"Precio vitrina inválido." };
  if (isNaN(punit) || punit < 0) return { ok:false, error:"Precio unitario inválido." };
  var stock = parseInt(obj.stock, 10);
  if (isNaN(stock) || stock < 0) stock = 0;
  var nuevo = {
    id: id,
    nombre: nombre,
    desc: (obj.desc || "").toString().trim() || nombre,
    pbase: pbase,
    punit: punit,
    stock: stock,
    agotado: stock === 0,
    tipo: obj.tipo === "extintor" ? "extintor" : "accesorio",
    _extra: true   // marca: producto añadido por admin (borrable)
  };
  var extra = pfCargarProductosExtra();
  extra.push(nuevo);
  localStorage.setItem(PF_PRODUCTOS_EXTRA_KEY, JSON.stringify(extra));
  PF_PRODUCTOS_DIST.push(nuevo);
  return { ok:true, producto:nuevo };
}

function pfEliminarProductoCatalogo(id) {
  var extra = pfCargarProductosExtra().filter(function(p){ return p.id !== id; });
  localStorage.setItem(PF_PRODUCTOS_EXTRA_KEY, JSON.stringify(extra));
  // quitar de memoria SOLO si es un producto extra (los hardcodeados nunca se borran)
  var idx = PF_PRODUCTOS_DIST.findIndex(function(p){ return p.id === id && p._extra; });
  if (idx !== -1) PF_PRODUCTOS_DIST.splice(idx, 1);
}

// Fusionar los extra al cargar (de-dup por id; un id hardcodeado siempre gana)
(function() {
  var extra = pfCargarProductosExtra();
  for (var i = 0; i < extra.length; i++) {
    var e = extra[i];
    if (e && e.id && !pfIdProductoExiste(e.id)) {
      e._extra = true;
      PF_PRODUCTOS_DIST.push(e);
    }
  }
})();
...
function pfHacerPDFProforma(prof) {
  ...
  var VERDE = [22, 101, 52];
  var AMARILLO = [245, 193, 7];   // amarillo PyroShield (fallback texto logo)
  var VBG   = [220, 252, 231];
  ...
  // ── HEADER ──
  y = 5;
  // Logo PyroShield: badge NEGRO + imagen; fallback "PYS" amarillo si no carga (#23)
  setFill(NEGRO); doc.roundedRect(MAR, y, 30, 30, 3, 3, "F");
  var _pysOk = false;
  if (window.PYS_LOGO_B64) {
    try { doc.addImage(window.PYS_LOGO_B64,"PNG",MAR+1,y+1,28,28); _pysOk = true; } catch(e){ _pysOk = false; }
  }
  if (!_pysOk) {
    doc.setFont("helvetica","bold"); doc.setFontSize(13); setTxt(AMARILLO);
