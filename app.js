...
function resetFotosUI(){
  for(var n=1;n<=3;n++){if(FD[n]){try{URL.revokeObjectURL(FD[n]);}catch(ex){}}}
...
function hacerPDF(fc){
  try{
    ...
    var tablaYi=y-rH;y+=tRows*rH;dR();doc.setLineWidth(0.5);doc.rect(ML,tablaYi,CW,rH*(tRows+1),"S");y+=8;
    // Párrafo garantía
