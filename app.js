// ═══════════════════════════════════════════════════════════
//  PREVIFUEGO FIELD — app.js  v2.0
//  Fase 2: Puntos dinámicos desde Google Sheets
// ═══════════════════════════════════════════════════════════

// ── CONFIG ──────────────────────────────────────────────────
var SCRIPT_URL = "https://script.google.com/macros/s/AKfycbzPY2DhypwIrP1M9sUTJNpWRkofw6PBCDh6dzo9sMy75rBQB4T4ehmnRMoaXaUbuDL9/exec";
...
function initLogos(){
  ...
  ["logo-img-s1","logo-img-shist","logo-img-sperf"].forEach(function(id){
    var img = document.getElementById(id);
    if(img) img.src = src;
  });
}

// ── CARGA DINÁMICA DESDE SHEETS ──────────────────────────────
function cargarRecorrido(){
  mostrarCargando(true, "Cargando recorrido...", "Conectando con el servidor");

  var url = SCRIPT_URL + "?accion=recorrido&fecha=" + encodeURIComponent(fechaHoy());
  ...
  fetch(url)
    ...
    .then(function(data){
      ...
      if(data.ok){
        procesarRecorrido(data);
      } else if(data.sin_recorrido){
        mostrarSinRecorrido(data.fecha);
      } else {
        console.warn("Sheets:", data.error || "error desconocido");
        mostrarSinRecorrido(null);
      }
    })
    .catch(function(err){
      mostrarCargando(false);
      console.warn("Sin conexión:", err.message);
      mostrarSinConexion();
    });
}

function procesarRecorrido(data){
  RECORRIDO_CARGADO = true;
  TECNICO_NOMBRE = data.tecnico || "Raúl Romero";

  // Actualizar nombre técnico en la UI
  var tsEl = document.querySelector("#s1 .ts");
  if(tsEl) tsEl.textContent = TECNICO_NOMBRE;
  var perfNom = document.querySelector("#sperf .sc div div:nth-child(2)");
  // nombre en perfil se actualiza en renderPerfil()

  // Convertir puntos del Sheet al formato interno
  PUNTOS = [];
  for(var i = 0; i < data.puntos.length; i++){
    var p = data.puntos[i];
    // Convertir extintores al formato que usa el PDF
    var exts = [];
    if(p.extintores && p.extintores.length > 0){
      for(var j = 0; j < p.extintores.length; j++){
        var e = p.extintores[j];
        exts.push({
          l: e.ubicacion || "Sin ubicación",
          t: e.tipo      || "PQS",
          w: e.capacidad || ""
        });
      }
    ...
    PUNTOS.push({
      ...
      n:      p.nombre    || p.codigo,
      cod:    p.codigo    || null,
      emp:    p.empresa   || "Cliente",
      dir:    p.nombre    || p.codigo,
      kfc:    p.kfc       || false,
      tipo:   p.tipo      || "M",
      done:   false,
      ext:    exts,
      ...
      ult_mant:     p.ult_mant     || "",
      prox_mant:    p.prox_mant    || "",
      mision:       p.mision       || "",
      sin_datos:    p.sin_datos    || false
    });
  }

  // Notas del recorrido (opcional — mostrar en banner si hay)
  if(data.notas){
    var notaEl = document.getElementById("s1-nota");
    if(notaEl){ notaEl.textContent = data.notas; notaEl.style.display="block"; }
  }

  renderPuntos();
}

function mostrarSinRecorrido(fecha){
  PUNTOS = [];
  var lista = document.getElementById("s1list");
  var msg = fecha
    ? "No hay recorrido publicado para hoy ("+fecha+").<br>Alejandro debe publicar el recorrido en el Excel."
    : "No hay recorrido publicado para hoy.<br>Alejandro debe publicar el recorrido en el Excel.";
  lista.innerHTML = '<div class="slbl">Sin recorrido</div><div style="margin:16px;padding:20px;background:#fff;border-radius:14px;border:1.5px dashed var(--bo);text-align:center;color:var(--g4);font-size:14px;line-height:1.7">📋 '+msg+'<br><br><button type="button" class="btn btn-g" style="font-size:13px;padding:10px 16px;width:auto;margin-top:4px" onclick="cargarRecorrido()">🔄 Reintentar</button></div>';
  document.getElementById("s1p").textContent = "0/0 misiones";
  document.getElementById("s1pf").style.width = "0%";
}

function mostrarSinConexion(){
  var lista = document.getElementById("s1list");
  lista.innerHTML = '<div class="slbl">Sin conexión</div><div style="margin:16px;padding:20px;background:#fff;border-radius:14px;border:1.5px dashed var(--bo);text-align:center;color:var(--g4);font-size:14px;line-height:1.7">📡 No hay conexión a internet.<br>Verifica los datos móviles e intenta de nuevo.<br><br><button type="button" class="btn btn-r" style="font-size:13px;padding:10px 16px;width:auto;margin-top:4px" onclick="cargarRecorrido()">🔄 Reintentar</button></div>';
  document.getElementById("s1p").textContent = "Sin conexión";
  document.getElementById("s1pf").style.width = "0%";
}
...
function renderPuntos(){
  ...
  document.getElementById("s1pf").style.width = PUNTOS.length > 0 ? (comp/PUNTOS.length*100)+"%" : "0%";
  ...
  for(var i = 0; i < PUNTOS.length; i++){
    ...
    if(p.sin_datos)    tags += '<span class="tg" style="background:#FFF4EC;color:#C25A00">Sin datos</span>';
...
function abrirPunto(id){
  var found = null;
  for(var i = 0; i < PUNTOS.length; i++){
    if(PUNTOS[i].id === id){ found = PUNTOS[i]; break; }
  }
  PA = found;
  if(!PA) return;
  ...
  document.getElementById("s2e").textContent =
    (PA.tipo==="M"?"Mantenimiento":"Recarga")+" · "+
    now.toLocaleDateString("es-EC",{day:"numeric",month:"long",year:"numeric"});

  document.getElementById("s2m").textContent =
    PA.mision ||
    (PA.tipo === "M"
...
function resetFotosUI(){
  for(var n = 1; n <= 3; n++){
    if(FD[n]){ try{ URL.revokeObjectURL(FD[n]); }catch(ex){} }
  }
...
function hacerPDF(fc){
  try{
    ...
    var codBase = (PA.cod||PA.n.substring(0,4).toUpperCase()).replace(/-/g,"").replace(/ /g,"");
    ...
    var ptxt  = "INSPECCIÓN Y MANTENIMIENTO DE EXTINTORES PORTÁTILES UBICADOS EN LOS DISTINTOS PUNTOS ESTRATEGICOS DEL LOCAL "+cod2+" PERTENECIENTE A LA EMPRESA "+PA.emp+", UBICADO EN "+PA.dir+", SIGUIENDO LAS NORMAS NFPA10.";
    ...
    var items = [
      ...
      "INSPECCIÓN DE CABEZALES",
      "INSPECCIÓN DE CORNETAS / MANGUERAS",
      "PESAJE / TARA",
    ...
    dR(); doc.setLineWidth(0.5); doc.rect(ML,tablaYi,CW,rH*(tRows+1),"S"); y += 8;
    ...
    var evTit = "EVIDENCIA FOTOGRÁFICA — "+codEv;
    ...
    var nom = cf+"-MANT DE EXTINTORES - "+ANO+".pdf";
...
function renderPerfil(){
  var nom = document.getElementById("sperf-nom");
  var ver = document.getElementById("sperf-ver");
  if(nom) nom.textContent = TECNICO_NOMBRE;
  if(ver) ver.textContent = "Previfuego Field v2.0";
}
...
window.onload = function(){
  ...
  renderAccSel();
  renderPerfil();
  cargarRecorrido();   // ← Carga dinámica desde Sheets
};
